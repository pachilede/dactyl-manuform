...
(def sla_tolerance 0.33) ; Current range is (0.25, 0.38]
...
(def mx_clone_hole_hotswap  ;;Special hole for hotswap holes because the box has to be a bit bigger so it makes contact with the kalih cutout.
  (->>(difference 
        (union
          (->>(cube (+ keyswitch-height 3), (+ keyswitch-width 3.5), 5.1) (color blue))   ;;Main box that everything is cut from - search range (5, 5.5]
          )
